window.onload = function(){
	...
	Snake.prototype = {
		...
		randomPoint:function(initX,initY,endX,endY){	
			...
			q[0] = Math.floor(Math.random()*(endx-initx))+initx;
			q[1] = Math.floor(Math.random()*(endy-inity))+inity;
			return p ,q ;
		},
		//判断点是否在蛇身的任一位置,pos:从身上的哪个位置开始判断
		...
		main:function(){
			...
			if(this.snakeGrid.length<=10){
				...
				if(this.snakeGrid.length>4){		//控制蛇加速
					...
					else if(this.snakeGrid.lengrh==40){
						this.speed +=5;
					}
					this.move();
				}
				...
				if(isEnd){
					...
					if(confirm(msg+"你拿到了："+score+"双AJ13 是否重新开始？")){
						this.reset();
					}
					return false;
				}
